...
def save_stations_batch(stations, batch_size=BATCH_SIZE):
    ...
    for offset in range(0, len(stations), batch_size):
        ...
        # Progress bar pour le batch courant
        for s in tqdm(batch, desc=f"Batch {offset // batch_size + 1}/{(len(stations) + batch_size - 1) // batch_size}",
                      unit="station"):
            with transaction.atomic():
                ...
                messages_list.append(f"{action} : {radio.name} (Pays : {radio.country})")
...
def radio_search(request):
    ...
    Actualise la base par batch pour les pays sélectionnés.
    """
    ...
    # Actualisation par batch même si des radios existent
    for country in selected_countries:
        stations = fetch_stations_by_country(country=country)
        save_stations_batch(stations, batch_size=BATCH_SIZE)

    # Filtrage après mise à jour
...
def radio_refresh_ajax(request):
    ...
    # Récupération des stations pour le pays
    stations = fetch_stations_by_country(country=country)
    total = len(stations)
    ...
    # Préparer indices pour le prochain batch ou pays
